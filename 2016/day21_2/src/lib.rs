...
fn rotate_based_on_letter_inv(s: &str, letter: char) -> Result<String, OperationErr> {
    let mut char_vec: Vec<_> = s.chars().collect();
    let len = char_vec.len();
    if ![1, 2, 3, 4, 6, 8].contains(&len) {
        return Err(OperationErr::NoInverse);
    }
    for _ in 0..len {
        if s == rotate_based_on_letter(&String::from_iter(&char_vec), letter)? {
            return Ok(char_vec.iter().collect());
        }
        char_vec.rotate_right(1);
    }
    panic!("function must return during one of the iterations")
}
...
pub fn apply_operations(s: &str, ops: &[Operation]) -> Result<String, ApplyOperationErr> {
    ...
    for op in ops {
        let result = match op {
            ...
            Operation::RotateBasedOnLetterInv(c) => rotate_based_on_letter_inv(&s, *c),
...
mod tests {
    ...
    #[test]
    fn rotate_based_on_letter_inv_test_1() {
        let s = "abcdefgh";
        assert_eq!(
            rotate_based_on_letter_inv(s, 'e'),
            Ok("habcdefg".to_owned())
        );
    }

    #[test]
    fn rotate_based_on_letter_inv_test_2() {
        let s = "abcdefgh";
        assert_eq!(
            rotate_based_on_letter_inv(s, 'h'),
            Ok("efghabcd".to_owned())
        );
    }

    #[test]
    fn rotate_based_on_letter_inv_test_3() {
        let s = "abcdefgh";
        assert_eq!(
            rotate_based_on_letter_inv(s, 'd'),
            Ok("cdefghab".to_owned())
        );
    }

    #[test]
    fn rotate_based_on_letter_inv_test_4() {
        let s = "abcdefgh";
        assert_eq!(
            rotate_based_on_letter_inv(s, 'r'),
            Err(OperationErr::LetterNotFound { letter: 'r' })
        );
    }

    #[test]
    fn rotate_based_on_letter_inv_test_5() {
        let s = "abcdefg";
        assert_eq!(
